...
from pedalboard import Distortion, Gain, LowpassFilter, Reverb, Delay
...
# Missing music files in your filepath? Download them from Jules' gdrive
def initialize_filepaths():
    music_paths["healthy_forest"] = (where_we_at_path + MUSIC_FOLDER + "\\HEALTHY") # Looks like "path/audio/HEALTHY"
    ...
    # audio_paths["climate_change"] = (where_we_at_path + MUSIC_FOLDER + "\\CLIMATE_CHANGE")
...
def set_scene(new_scene):
    ...
    subfolder_path = music_paths[new_scene]
    filenames = os.listdir(subfolder_path)
    file_to_play = random.sample(filenames, 1)
    load_audio(file_to_play[0], subfolder_path + "\\" + file_to_play[0])
    play_audio(file_to_play[0])
...
def update(scene):
    ...
    for filename in remaining_times:
        if remaining_times[filename] < FADE_TIME_s and file_active[filename] is 1:
            ...
            subfolder_path = music_paths[scene]
            filenames = os.listdir(subfolder_path)
...
# LOWPASS_FILTER_MIN_Hz = 200
# LOWPASS_FILTER_MAX_Hz = 21500

# def get_effect_knob_vals():
#     vals = [0,0,0]
#     for val in vals:
#         # Get value from parameter.py?
#         # climate, human interaction, entropy
#         val = 0
#     return vals

# def add_effect(board, effect_name):
#     # Different effect ideas: Pitch shift
#     # TODO Initialize parameters to current knob value
#     if effect_name is 'distortion':
#         val = 0
#         board.append(Distortion(drive_db=val))
#         board.append(Gain(gain_db=-val))
#         # Adjust parameter with: drive_db
#     elif effect_name is 'lpf':
#         val = 20000
#         board.append(LowpassFilter(cutoff_frequency_hz=val))
#         # Adjust parameter with: cutoff_frequency_hz
#     elif effect_name is 'reverb':
#         board.append(Reverb()) # what to set threshold??
#         # Adjust parameter with: threshold_db, ratio, attack_ms, release_ms
#     else:
#         print('This effect is not implemented')
...
def play_audio(filename, isMusic=False):
    ...
    # Effects
    # board = Pedalboard([]) # init?
    # if isMusic is not False:
    #     add_effect(board, 'distortion')
    #     add_effect(board, 'lpf')
    #     add_effect(board, 'reverb')
    ...
    def write_audio():
        ...
        while current_index < audio_len[filename]:
            ...
            #     effected = board(chunk, samplerates[filename], reset=False)
